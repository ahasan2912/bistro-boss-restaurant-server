  ...
  try {
    ...
    app.get('/admin-state', verifyToken, verifyAdmin,  async (req, res) => {
      ...
      const result = await paymentCollection.aggregate([
        {
          $group: {
            _id: null, //_id: null means that not specific id, means all _id.
            totalRevenue: {
              $sum: '$price'
            }
          }
     ...
     * 2. for every menuItem (which is an array), go find the item from menu collection
     * 3. for every item in menu collection that you found from a element entry (document)
     */
    ...
    app.get('/order-stats', verifyToken, verifyAdmin, async (req, res) => {
      const result = await paymentCollection.aggregate([
        ...
        {
          $group: {
            _id: '$MenuItemId.category',
            quantity: { $sum: 1 },
            revenue: { $sum: '$MenuItemId.price'}
          }
        },
        {
          $project: {
